...
class TextNormalizer(BaseEstimator, TransformerMixin):
    """Transformer to normalize text using stemming or lemmatization."""
    ...
    def transform(self, X):
        """Apply text normalization to a series of text documents."""
        import re
        
        # Function to normalize a single document
        ...
        normalized = [normalize_doc(doc) for doc in X]
        return normalized
...
class NormalizingTextCombiner(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if hasattr(X, 'iloc'):  # Check if it's a DataFrame
            ...
            combined = X  # If not a DataFrame, assume it's already processed
...
def main():
    ...
    # Get the best hyperparameters based on previous experiments
    model_params = {
        'max_features': 5000,
        'ngram_range': (1, 2),
        'min_df': 5,
